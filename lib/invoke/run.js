...
// Added parentContainer to detect if we are in a docker-in-docker situation
module.exports = (opt) => {
  const options = opt || {}
  ...
  const parentContainer = process.env.DOCKER ? process.env.HOSTNAME : false
  ...
  const args = ['run']
    .concat(parentContainer ? ['--volumes-from', `${parentContainer}`] : [])
